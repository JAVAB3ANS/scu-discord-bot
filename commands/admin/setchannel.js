...
module.exports = class setChannelCommand extends Command {
  ...
  async run(client, message, { type, channel }) {
    if (this.client.config.channels.hasOwnProperty(type)) {
      ...
      sendMessage(client, client.config.channels.auditlogs, { embed: { title: `Channel Updated`, description: `${type} => #${channel.name}`, color: this.client.config.school_color }});
    } else {
        sendMessage(client, client.config.channels.auditlogs, { embed: { title: "Could not find that channel key", description: message, color: "RED"}});
    }
  }
};